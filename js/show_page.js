...
function ShowoPage(Tabstart,Tabend,CurrPageFont1,CurrPageFont2,PageCountFont1,PageCountFont2,PrePageFont1,PrePageFont2,RecCountFont1,RecCountFont2,FirstFont,PrevFont,NextFont,LastFont,Jump,PageNumFont1,PageNumFont2,PageNumFont3,PageNumFont4,LinkFont1,LinkFont2,LinkFont3,LinkFont4,iRecCount,iRecPerPage,iPageNum) {
	...
	if (CurrPage<=1&&PageCount<=1) {
		...
		FirstPageUrl="&nbsp;"+LinkFont3+FirstFont+LinkFont4+"&nbsp;";
		PrevPageUrl="&nbsp;"+LinkFont3+PrevFont+LinkFont4+"&nbsp;";
		NextPageUrl="&nbsp;<A href=\""+url+"Page="+nextpage+"\"  class='hs'>"+LinkFont1+NextFont+LinkFont2+"</A>&nbsp;";
		LastPageUrl="&nbsp;<A href=\""+url+"Page="+PageCount+"\"  class='hs'>"+LinkFont1+LastFont+LinkFont2+"</A>&nbsp;";
	}
	else if (CurrPage==PageCount) {
		FirstPageUrl="&nbsp;<A href=\""+url+"Page=1\"  class='hs'>"+LinkFont1+FirstFont+LinkFont2+"</A>&nbsp;";
		PrevPageUrl="&nbsp;<A href=\""+url+"Page="+prevpage+"\"  class='hs'>"+LinkFont1+PrevFont+LinkFont2+"</A>&nbsp;";
		NextPageUrl="&nbsp;"+LinkFont3+NextFont+LinkFont4+"&nbsp;";
		LastPageUrl="&nbsp;"+LinkFont3+LastFont+LinkFont4+"&nbsp;";
	}
	else {
	...
	if (CurrPage>=1) {
		for (i=PageStart;i<=PageEnd;i++) {
			if (i!=CurrPage) {
				document.write ("&nbsp;<A href=\""+url+"Page="+i+"\"  class='hs'>"+PageNumFont1+i+PageNumFont2+"</A>&nbsp;");
			}
			else {
	...
	document.write ("<INPUT type=\"text\" id=\"Page\" size=\"2\" maxlength=\"10\"  onkeydown=\"if (event.keyCode==13) CheckPage("+PageCount+")\"><INPUT type=\"button\" value=\"Go\" onClick=\"CheckPage("+PageCount+")\">"+Tabend);
}