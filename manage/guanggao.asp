...
<body>
		...
		<%
		sql="select * from guanggao order by id asc"
...
%>
                <TR <%if i mod 2=0 then%>bgColor="#fffff0"<%else%>bgcolor="#F6F6F6"<%end if%>>
                  ...
                  <TD align="left" ><A href="guanggao.asp?action=edit&id=<%=rs("id")%>"><%=rs("title")%></A></TD>
                  <TD align="middle" ><A title="修改" href="?action=edit&id=<%=rs("id")%>&linkclass=<%=rs("newsclass")%>"><IMG height="20" src="images/icon_edit.gif" width="20" border="0"></A></TD>
                </TR>
<%
RowCount=RowCount-1
rs.movenext
...
<form name="formc" method="post" action="?action=aa"><table width="90%" border=0 align=center cellSpacing=1 class="navi">
    ...
    <tr>
      <td width="73" height="20" class="front3"><div align="right">标题：</div></td>
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tbody>
            <tr>
              <td valign="bottom" class="SubTitle"><img height="20" src="images/icon_info.gif" width="20" align="absmiddle" /> 修改广告</td>
            </tr>
            <tr>
...
<table width="90%" border=0 align=center cellSpacing=1 class="navi">
    ...
    <tr>
      <td width="137" height="20" align="right" class="front3"><div align="right">标题：</div></td>
      <td width="1111" class="STYLE5"><input name="title" type="text" id="title" value="<%=rs("title")%>"></td>
    </tr>
    <tr>
      <td width="137" height="20" valign="top" bgcolor="#FFFFFF" class="STYLE5"><div align="right">内容：</div></td>
...
if request("action")="editsave" then
	...
	set rs=nothing
	set rs1=nothing
	Response.Write "<script>alert('您已经成功修改');location='guanggao.asp'</script>"
end if
