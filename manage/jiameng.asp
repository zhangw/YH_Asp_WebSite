...
<body>
          ...
          <%if request("action")="" then%><TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
              <TBODY>
                <TR>
                  <TD><IMG height="20" src="images/icon_info.gif" width="20" align="absMiddle">单页信息的增加、修改和删除。</TD>
                  <TD align="right"></TD>
                </TR>
                <TR>
                  <TD></TD>
            ...
            <TABLE width="90%" border="0" align="center" cellPadding="1" cellSpacing="1" bgColor="#c0c0c0">
              <TBODY>
				...
				<%
sql="select * from jiameng  order by id asc"
...
i=i+1
%>
                <TR bgcolor="#ffffff"  onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#F0E27D'">
                  <TD align="middle" ><div align="center"><%=i%></div></TD>
                  <TD align="left" ><A title="修改" href="?action=edit&id=<%=rs("id")%>&pid=<%=request("pid")%>"><%=rs("title")%></A></TD>
...
sql="select * from jiameng where id="&int(request("pid"))
Set rs= Server.CreateObject("ADODB.Recordset")
rs.open sql,conn,1,3
if not rs.eof then
...
if request("action")="aa" then
	...
	rs.close
	set rs=nothing		
	Response.Write "<script>alert('您已经成功添加');location='jiameng.asp?pid="&request("pid")&"'</script>"
end if	
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from jiameng where id="& request("id")
...
<form name="formc" method="post" action="?action=editsave&pid=<%=request("pid")%>&id=<%=request("id")%>">
  <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td height="20" valign="top" bgcolor="#FFFFFF" class="STYLE5"><div align="right"><br />
      内容：</div></td>
      <td height="250" align="left" bgcolor="#FFFFFF" class="STYLE5"><textarea name="theme" style="display:none"><%=rs("theme")%></textarea><iframe id="editor2" src="../Editor/eWebEditor.asp?id=theme" frameborder=1 scrolling=no width="550" height="405"></iframe>	</td>
    </tr>
    <tr bgcolor="#A4B6D7">
      <td height="20" bgcolor="#FFFFFF">&nbsp;</td>
      <td align="left" bgcolor="#FFFFFF"><input type="submit" name="Submit" value="确定"></td>
...
if request("action")="editsave" then
	...
	rs.close
	set rs=nothing	
	Response.Write "<script>alert('您已经成功修改');location='jiameng.asp?pid="&request("pid")&"'</script>"
end if
if request("action")="del" then
		...
		rs.close
		set rs=nothing
		Response.Write "<script>alert('删除成功');location='jiameng.asp?pid="&request("pid")&"'</script>"
end if
%>		
</body>
</html>
