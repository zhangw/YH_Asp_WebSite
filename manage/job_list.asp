...
<body>
 ...
 <form id="form2" name="form2" method="post" action="?action=del&newsid=<%=rs("newsid")%>&page=<%=int(request("page"))%>"> <tr bgcolor="#e0e9f8" class="unnamed1">
    ...
    <th width="453" height="20" bgcolor="#e0e9f8"  class="STYLE5"><div align="center">标题</div></th>
  ...
  <%     set rs=server.createobject("adodb.recordset")
	sql="select * from job where title<>'' " 
...
%>
  ...
  <tr bgcolor="#ffffff" class="unnamed1" onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#F0E27D'">
    ...
    <td width="453" height="20"  class="STYLE5" style="padding-left:5px;"><%if len(rs("title"))>32 then%><%=mid(rs("title"),1,32)%>...<%else%><%=rs("title")%><%end if%></td>
	<%
...
%>
  <tr bgcolor="#ffffff" class="unnamed1" onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#F0E27D'">
    <td height="20" colspan="5" align="right"  class="STYLE5" style="padding-left:5px;"><a href="#" onclick="cc('form2',1)">全选</a> <a href="#" onclick="cc('form2',0)">全不选</a> <a href="#" onclick="cc('form2',2)">反选</a>&nbsp;
...
&nbsp;总数<%= rs.recordcount %>条</div></td>
            ...
            <TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
              <TBODY>
                ...
                <TR>
                  <TD><IMG height="20" src="images/icon_new.gif" width="20" align="absMiddle"> <A href="job_add.asp">新增信息</A></TD>
                </TR>
              </TBODY>
</TABLE>
<%end if%>	
...
<table width="90%" border=0 align=center cellSpacing=1 class="navi">
    ...
    <tr>
      ...
      <td colspan="2" bgcolor="#FFFFFF">
	                ...
	                <iframe id="editor2" src="../pic/eWebEditor.asp?id=theme" frameborder=1 scrolling=no width="560" height="405"></iframe></td>
    </tr>
    <tr bgcolor="#A4B6D7">
...
if request("action")="editsave" then
	...
	rs("ms")=request("ms")
	rs("content")=theme
	rs("newsid")=request("BigClassName")
	...
	Response.Write "<script>alert('您已经成功修改');location='job_list.asp?page="&int(request("page"))&"&titles="&request("titles")&"&newsid="&request("newsid")&"'</script>"
end if
if request("action")="del" then
		sql="DELETE  from job where id in("& request("id")&")" 
		conn.execute(sql)
	response.Redirect "job_list.asp?page="&request("page")&"&titles="&request("titles")&"&newsid="&request("newsid")
end if
