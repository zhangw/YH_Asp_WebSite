...
if request("action")="aa" then
    set rs=server.createobject("adodb.recordset")
	sql="select * from dowlodid order by dlid desc" 
	rs.open sql,conn,1,3
	...
	set rs1=nothing
	set rs=nothing
	Response.Write "<script>alert('您已经成功添加');location='hy_class1.asp?lb="&cint(request("lb"))&"'</script>"
end if	
'###############################################################
'                          类别的编辑
'###############################################################
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from dowlodid where id="& request("id")
...
<form name="formc" method="post" action="?action=editsave&id=<%=request("id")%>&dowlodid=<%=rs("dlid")%>">
<table width="90%" border=0 align=center cellSpacing=1 class="navi">
...
if request("action")="editsave" then
    set rs=server.createobject("adodb.recordset")
	sql="select * from dowlodid where dlid="&request("dowlodid") 
	rs.open sql,conn,1,3
	rs("title")=request("title")
	rs.update
	rs.close
	set rs=nothing
	Response.Write "<script>alert('您已经成功修改');location='hy_class1.asp'</script>"
end if
'###############################################################
'                          类别的删除
'###############################################################
if request("action")="del" then
		...
		rs.close
		set rs=nothing
		Response.Write "<script>alert('您已经成功删除');location='hy_class1.asp?lb="&cint(request("lb"))&"'</script>"
end if
%>		
</body>
</html>
