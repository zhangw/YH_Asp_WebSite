...
<body>
          ...
          <%if request("action")="" or request("action")="list" then%><TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
              <TBODY>
                <TR>
                  <TD><IMG height="20" src="images/icon_info.gif" width="20" align="absMiddle">新闻类别的增加,删除和修改.</TD>
                  <TD align="right">&nbsp;</TD>
                </TR>
                <TR>
                  <TD height="5"></TD>
...
%>
                <TR <%if i mod 2=0 then%>bgColor="#fffff0"<%else%>bgcolor="#F6F6F6"<%end if%>>
                  ...
                  <TD align="left" ><A href="?action=edit&id=<%=rs("id")%>&newsid=<%=rs("newsclass")%>"><%=rs("title")%></A></TD>
                  <TD align="middle" ><A title="修改" href="?action=edit&id=<%=rs("id")%>&newsid=<%=rs("newsclass")%>"><IMG height="20" src="images/icon_edit.gif" width="20" border="0"></A><a   href="#"onclick="javascript:if   (confirm('确实要删除吗'))   href='?action=del&id=<%=rs("id")%>&newsid=<%=rs("newsclass")%>';else   return;"><IMG src="images/icon_del.gif" alt="" width="20" height="20" border="0" title="删除"></A></TD>
                </TR>
<%
RowCount=RowCount-1
rs.movenext
...
if request("action")="aa" then
	...
	sql="select * from newsid order by id desc" 
	rs_sel.open sql,conn,1,3
	
    set rs=server.createobject("adodb.recordset")
	sql="select * from newsid " 
	rs.open sql,conn,1,3
	rs.addnew
	...
	Response.Write "<script>alert('您已经成功添加');location='news_class.asp'</script>"
end if	
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from newsid where id="& request("id")
...
if request("action")="editsave" then
	...
	Response.Write "<script>alert('您已经成功修改');location='news_class.asp'</script>"
end if
if request("action")="del" then
		...
		Response.Write "<script>alert('删除成功');location='news_class.asp'</script>"
end if
%>		
</body>
