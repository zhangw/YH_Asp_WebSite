...
<%
sql="select  * from product_class  order by id asc"
...
<form id="form2" name="form2" method="post" action="?action=del&newsid=<%=request("newsid")%>&titles=<%=request("titles")%>&page=<%=int(request("page"))%>">
  ...
  <!--
    <th width="284" bgcolor="#e0e9f8"  class="STYLE5">品牌</th>
    <th width="284" bgcolor="#e0e9f8"  class="STYLE5">等级</th>
    <th width="284" bgcolor="#e0e9f8"  class="STYLE5">型号</th>-->
...
%>
  <tr bgcolor="#ffffff"  onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#F0E27D'" class="unnamed1">
   <td width="24"   class="STYLE5" style="padding-left:5px;">
    ...
    <td width="284" align="center"  class="STYLE5"><%=str%></td>
<!--    <td width="284" align="center"  class="STYLE5"><%=rs("pp")%></td>
    <td width="284" align="center"  class="STYLE5"><%=rs("dj")%></td>
    <td width="284" align="center"  class="STYLE5"><%=rs("xh")%></td>-->
    ...
    <td width="195" ><div align="center"><A title="修改" href="?action=edit&id=<%=rs("id")%>&bigclassname=<%=rs("productclass1")%>&title=<%=rs("title")%>"><IMG height="20" src="images/icon_edit.gif" width="20" border="0"></A><a   href="#" onclick="javascript:if   (confirm('确实要删除吗'))   href='?action=del&id=<%=rs("id")%>&newsid=<%=request("newsid")%>&titles=<%=request("titles")%>';else   return;"><IMG src="images/icon_del.gif" alt="" width="20" height="20" border="0" title="删除"></A></div></td>
  </tr>
  <%RowCount=RowCount-1
rs.movenext
loop
%>
  <tr bgcolor="#ffffff" class="unnamed1" onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#F0E27D'">
    <td height="20" colspan="9" align="right"  class="STYLE5" style="padding-left:5px;"><a href="#" onclick="cc('form2',1)">全选</a> <a href="#" onclick="cc('form2',0)">全不选</a> <a href="#" onclick="cc('form2',2)">反选</a>&nbsp;
...
&nbsp;总数<%= rs.recordcount %>条</div></td>
            ...
            <TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
              <TBODY>
                ...
                <TR>
                  <TD><IMG height="20" src="images/icon_new.gif" width="20" align="absMiddle"> <A href="product_add.asp?BigClassName=<%=request("BigClassName")%>">新增信息</A></TD>
                </TR>
              </TBODY>
</TABLE>
<%end if%>	
...
%>
       ...
       <select name="SmallClassName" >
          ...
          <%
			...
			<!--    <tr>
      <td width="126"  height="20" align="right" bgcolor="#FFFFFF" class="STYLE5"><div align="right">参数1：</div></td>
      <td width="934" height="20" colspan="2" bgcolor="#FFFFFF"><input name="price" type="text" id="price" style="width:300px;" value="<%=rs("price")%>"></td>
    </tr>
<tr>
      <td width="126"  height="20" align="right" bgcolor="#FFFFFF" class="STYLE5"><div align="right">参数2：</div></td>
      <td width="934" height="20" colspan="2" bgcolor="#FFFFFF"><input name="pp" type="text" id="pp" style="width:300px;" value="<%=rs("pp")%>"></td>
    </tr>
	    <tr>
      <td width="126"  height="20" align="right" bgcolor="#FFFFFF" class="STYLE5"><div align="right">参数3：</div></td>
      <td width="934" height="20" colspan="2" bgcolor="#FFFFFF"><input name="dj" type="text" id="dj" style="width:300px;" value="<%=rs("dj")%>"></td>
    </tr>
	    <tr>
      <td width="126"  height="20" align="right" bgcolor="#FFFFFF" class="STYLE5"><div align="right">参数4：</div></td>
      <td width="934" height="20" colspan="2" bgcolor="#FFFFFF"><input name="cd" type="text" id="cd" style="width:300px;" value="<%=rs("cd")%>"></td>
    </tr>
	    <tr>
      <td width="126"  height="20" align="right" bgcolor="#FFFFFF" class="STYLE5"><div align="right">参数5：</div></td>
      <td width="934" height="20" colspan="2" bgcolor="#FFFFFF"><input name="xh" type="text" id="xh" style="width:300px;" value="<%=rs("xh")%>"></td>
    </tr>-->
    <tr>
     ...
     <tr> 
                  ...
                  <td height="27"> <input name="defaultpicurl" type="text" id="defaultpicurl"  value="<%=rs("pic")%>" size="40" maxlength="120"><input name="UploadFiles" type="hidden" id="UploadFiles2">(请复制上传后的地址填入)</td>
                </tr>
...
if request("action")="editsave" then
		...
		rs1("pic")=request("defaultpicurl")'图片
		rs1("time")=now()
		rs1.update
		'rs1.requery
		rs1.close
		set rs1=nothing
		Response.Write "<script>alert('您已经成功修改');location='product_list.asp'</script>"
end if
if request("action")="del" then
		sql="DELETE  from product where id in("& request("id")&")" 
		conn.execute(sql)
response.Redirect "product_list.asp?page="&request("page")&"&newsid="&request("newsid")&"&titles="&request("titles")
end if
if request("action")="tuij" then
	...
	rs.close
	set rs=nothing	
	Response.Redirect "product_list.asp?page="&request("page")
end if
...
if request("action")="sy" then
		set rs=server.createObject("ADODB.Recordset")
		if request("newsid")="" then
		sql="select * from product where rootid<"& request("rootid") &" order by rootid desc"
		else
		sql="select * from product where rootid<"& request("rootid") &" and productclass1="&request("newsid")&" order by rootid desc"
		end if
		rs.open sql,conn,2,3
		if not rs.eof then
		rootid=rs("rootid")
		set rs1=server.createObject("ADODB.Recordset")
		sql="select * from product where rootid="& request("rootid") 
...
if request("action")="xy" then
		set rs=server.createObject("ADODB.Recordset")
		if request("newsid")="" then
		sql="select * from product where rootid>"& request("rootid") &" order by rootid asc"
		else
		sql="select * from product where rootid>"& request("rootid")&" and productclass1="&request("newsid")&" order by rootid asc"
		end if
		rs.open sql,conn,2,3
		if not rs.eof then
		rootid=rs("rootid")
