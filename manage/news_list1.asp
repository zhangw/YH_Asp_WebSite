...
<body>
 ...
 <form id="form2" name="form2" method="post" action="?action=del&newsid=<%=rs("newsid")%>&page=<%=int(request("page"))%>"> <tr bgcolor="#e0e9f8" class="unnamed1">
  ...
  <%     set rs=server.createobject("adodb.recordset")
	sql="select * from news where title<>'' and newsid=2" 
...
&nbsp;总数<%= rs.recordcount %>条</div></td>
            ...
            <TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
              <TBODY>
                ...
                <TR>
                  <TD><IMG height="20" src="images/icon_new.gif" width="20" align="absMiddle"> <A href="news_add1.asp">新增信息</A></TD>
                </TR>
              </TBODY>
</TABLE>
<%end if%>	
...
if request("action")="editsave" then
	...
	Response.Write "<script>alert('您已经成功修改');location='news_list1.asp?page="&int(request("page"))&"&titles="&request("titles")&"&newsid="&request("newsid")&"'</script>"
end if
if request("action")="del" then
		sql="DELETE  from news where id in("& request("id")&")" 
		conn.execute(sql)
	response.Redirect "news_list1.asp?page="&request("page")&"&titles="&request("titles")&"&newsid="&request("newsid")
end if
