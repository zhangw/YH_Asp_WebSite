...
<body>
          ...
          <%if request("action")="" or request("action")="list" then%><TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
              <TBODY>
                <TR>
                  <TD><IMG height="20" src="images/icon_info.gif" width="20" align="absMiddle"> 网页建设内容的增加、修改和删除。</TD>
                  <TD align="right">&nbsp;</TD>
                </TR>
                <TR>
                  <TD height="5"></TD>
...
if request("action")="aa" then
    set rs=server.createobject("adodb.recordset")
	sql="select * from server_class order by classname desc" 
	rs.open sql,conn,1,3
	...
	rs1("classname")=rs("classname")+1
	rs1.update
	rs1.close
	set rs=nothing
	set rs1=nothing		
	Response.Write "<script>alert('您已经成功添加');location='kefu_class.asp'</script>"
end if
'##########################################################
'						类别的编辑
'##########################################################
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from server_class where id="& request("id")
...
if request("action")="editsave" then
	...
	rs.close
	set rs=nothing
	Response.Write "<script>alert('您已经成功修改');location='kefu_class.asp'</script>"
end if
'##########################################################
'						类别的删除
'##########################################################
if request("action")="del" then
		...
		rs.close
		set rs=nothing
		Response.Write "<script>alert('删除成功');location='kefu_class.asp'</script>"
end if
%>		
</body>
</html>
