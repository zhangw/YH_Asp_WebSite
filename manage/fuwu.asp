          ...
          <%if request("action")="" then%><TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
            ...
            <TABLE width="90%" border="0" align="center" cellPadding="1" cellSpacing="1" bgColor="#c0c0c0">
              <TBODY>
				...
				<%
sql="select * from fuwu order by id asc"
...
if request("action")="aa" then
	...
	rs.close
	set rs=nothing		
	Response.Write "<script>alert('鎮ㄥ凡缁忔垚鍔熸坊鍔?);location='fuwu.asp'</script>"
end if	
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from fuwu where id="& request("id")
...
if request("action")="editsave" then
	...
	rs.close
	set rs=nothing	
	Response.Write "<script>alert('鎮ㄥ凡缁忔垚鍔熶慨鏀?);location='fuwu.asp'</script>"
end if
if request("action")="del" then
		...
		rs.close
		set rs=nothing
		Response.Write "<script>alert('鍒犻櫎鎴愬姛');location='fuwu.asp'</script>"
end if
%>		
</body>
</html>
