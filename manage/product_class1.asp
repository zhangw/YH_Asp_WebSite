...
<body>
          ...
          <%if request("action")="" then%><TABLE width="90%" border="0" align="center" cellPadding="0" cellSpacing="0">
              <TBODY>
                <TR>
                  <TD><IMG height="20" src="images/icon_info.gif" width="20" align="absMiddle"> 产品类别的增加、修改和删除。</TD>
                  <TD align="right"></TD>
                </TR>
                <TR>
                  <TD></TD>
...
sql="select * from product_class where pid="&int(request("pid"))&" order by id asc"
Set rs= Server.CreateObject("ADODB.Recordset")
rs.open sql,conn,1,3
i=0 
do while not rs.eof
sql="select * from product_class where id="&rs("class")
...
%>
                <TR bgColor="#fffff0">
                  ...
                  <TD align="left" bgColor="#fffff0"><%=rs("classname")%></TD>
...
sql="select * from product_class where id="&int(request("pid"))
Set rs= Server.CreateObject("ADODB.Recordset")
rs.open sql,conn,1,3
if not rs.eof then
%><a href="product_class.asp?pid=<%=rs("pid")%>">返回上一级</a><%end if%></TD>
                </TR>
              </TBODY>
</TABLE>
<%end if%>	
...
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from product_class where id="& request("id")
