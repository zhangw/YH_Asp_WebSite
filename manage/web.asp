            ...
            <TABLE width="90%" border="0" align="center" cellPadding="1" cellSpacing="1" bgColor="#c0c0c0">
              <TBODY>
                <TR bgColor="#e0e9f8">
                  ...
                  <TD align="middle" width="432"><strong>鏍囬</strong></TD>
                  <TD align="middle" width="432"><strong>绫诲埆</strong></TD>
                  <TD align="middle" width="50"><STRONG>鎿?浣?/STRONG></TD>
                </TR>
				<%
sql="select * from kefu order by id asc"
Set rs= Server.CreateObject("ADODB.Recordset")
rs.open sql,conn,1,3
i=0 
do while not rs.eof
sql="select * from kefu_class where id="&rs("class")
...
%>
                <TR bgColor="#fffff0">
                  ...
                  <TD align="left" bgColor="#fffff0"><%=rs1("class")%></TD>
  ...
  <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <span class="STYLE5"> (鏍囬璇蜂笉瑕佸ぇ浜?0瀛?</span></td>
	    ...
	    <tr bgcolor="#A4B6D7">
	      <td height="20" align="right" valign="middle" bgcolor="#FFFFFF" class="STYLE5">绫诲埆锛?/td>
	      <td height="20" align="left" valign="middle" bgcolor="#FFFFFF" class="STYLE5"><select name="class" id="class">
    <%
	set rs1=server.createobject("adodb.recordset")
	sql="select * from kefu_class "
	rs1.open sql,conn,1,3
	do while not rs1.eof
	%>
	 <option value="<%=rs1("id")%>"><%=rs1("class")%></option>
   <%
   rs1.movenext
   loop
   %>
...
if request("action")="aa" then
	...
	rs.close
	set rs=nothing		
	Response.Write "<script>alert('鎮ㄥ凡缁忔垚鍔熸坊鍔?);location='web.asp'</script>"
end if	
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from kefu where id="& request("id")
...
<form name="formc" method="post" action="?action=editsave&id=<%=request("id")%>">
  <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	do while not rs1.eof
	%>
	 <option value="<%=rs1("id")%>" <%if rs("class")=rs1("id") then%>selected="selected"<%end if%>><%=rs1("class")%></option>
   <%
   rs1.movenext
   loop
   %>
      ...
      <td height="20" valign="top" bgcolor="#FFFFFF" class="STYLE5"><div align="right"><br />
      鍐呭锛?/div></td>
      <td height="250" align="left" bgcolor="#FFFFFF" class="STYLE5"><textarea name="theme" style="visibility:hidden"><%=rs("theme")%></textarea><iframe id="editor2" src="../pic/eWebEditor.asp?id=theme" frameborder=1 scrolling=no width="550" height="405"></iframe>	</td>
    </tr>
    <tr bgcolor="#A4B6D7">
      <td height="20" bgcolor="#FFFFFF">&nbsp;</td>
      <td align="left" bgcolor="#FFFFFF"><input type="submit" name="Submit" value="纭畾"></td>
...
if request("action")="editsave" then
	...
	rs.close
	set rs=nothing	
	Response.Write "<script>alert('鎮ㄥ凡缁忔垚鍔熶慨鏀?);location='web.asp'</script>"
end if
if request("action")="del" then
		...
		rs.close
		set rs=nothing
		Response.Write "<script>alert('鍒犻櫎鎴愬姛');location='web.asp'</script>"
end if
%>		
</body>
</html>
