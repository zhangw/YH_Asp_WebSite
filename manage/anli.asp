...
<body>
			<table cellSpacing="0" cellPadding="0" width="100%" bgColor="#f1f1f1" border="0" >
				<tr>
					<td>
						<table cellSpacing="0" cellPadding="0" background="images/sheet_bk.gif" border="0" style="margin-top:5px;">
							<tr>
...
%>			
  ...
  <tr>
    <td height="20" ><div align="center" class="unnamed1">第<%= page %>页&nbsp; <a href="?page=1<%if request("title")<>"" then%>&title=<%=request("title")%><%end if%><%if request("theme")<>"" then%>&theme=<%=request("theme")%><%end if%><%if request("lb")<>"" then%>&lb=<%=cint(request("lb"))%><%end if%>" class="hh">首页</a> &nbsp;共<%=rs.PageCount%>页&nbsp;
            <% if page>1 then %>
            <a href="?page=<%= page-1 %><%if request("title")<>"" then%>&title=<%=request("title")%><%end if%><%if request("theme")<>"" then%>&theme=<%=request("theme")%><%end if%><%if request("lb")<>"" then%>&lb=<%=cint(request("lb"))%><%end if%>" class="hh" >上一页</a>
            <% else %>
        上一页
        <% end if %>
&nbsp;<span class="A3"> </span>
        <% if page<rs.pagecount then %>
        <a href="?page=<%=page+1%><%if request("title")<>"" then%>&title=<%=request("title")%><%end if%><%if request("theme")<>"" then%>&theme=<%=request("theme")%><%end if%><%if request("lb")<>"" then%>&lb=<%=cint(request("lb"))%><%end if%>" class="hh" >下一页</a>
        <% else %>
        下一页
        <% end if %>
...
if request("action")="aa" then
    set rs=server.createobject("adodb.recordset")
	sql="select * from productid order by productclass desc" 
	rs.open sql,conn,1,3
	...
	rs1("productclass")=rs("productclass")+1
	rs1.update
	rs1.close
	set rs1=nothing
	set rs=nothing
	Response.Write "<script>alert('您已经成功添加');location='anli.asp?lb="&cint(request("lb"))&"'</script>"
end if	
'###############################################################
'                          类别的编辑
'###############################################################
if request("action")="edit" then
	set rs=server.createObject("ADODB.Recordset")
	sql="select * from productid where id="& request("id")
...
<form name="formc" method="post" action="?action=editsave&id=<%=request("id")%>&dowlodid=<%=rs("productid")%>">
<table width="90%" border=0 align=center cellSpacing=1 class="navi">
...
if request("action")="editsave" then
	...
	rs.close
	set rs=nothing
	Response.Write "<script>alert('您已经成功修改');location='anli.asp'</script>"
end if
'###############################################################
'                          类别的删除
'###############################################################
if request("action")="del" then
		...
		rs.close
		set rs=nothing
		Response.Write "<script>alert('您已经成功删除');location='anli.asp?lb="&cint(request("lb"))&"'</script>"
end if
%>		
</body>
</html>
